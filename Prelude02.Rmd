...
First, load the package (we already installed it in <a href="Prelude01.html" target="_blank">Section 7 of Prelude 1</a>).
...
ggplot(data = BP_Flats, aes(y = Price_MillionHUF, fill = District)) +
  geom_boxplot() +
  labs(y = "Price in Million HUF", fill = "District") +
...
As we can see, it’s also possible to assign custom colors to different plot elements. You can find a good summary of available colors in `ggplot2` <a href="https://www.r-graph-gallery.com/ggplot2-color.html" target="_blank">here</a>.

In the parameters of the `theme` function, we can specify how we want every component of the plot to look. For an overview of the options, you can find a good reference <a href="https://ggplot2.tidyverse.org/reference/theme.html" target="_blank">on this link</a>.
...
Our stacked bar chart now has beautiful dark-toned colors! :) You can read more about color palettes and the deeper mysteries of color modification in `ggplot2` on <a href="http://www.sthda.com/english/wiki/ggplot2-colors-how-to-change-colors-automatically-and-manually" target="_blank">this</a> link.
...
But the beauty of R is that there are <a href="http://www.sthda.com/english/wiki/visualize-correlation-matrix-using-correlogram" target="_blank">many packages available</a> for visualizing the correlation matrix! So, the possibilities are endless! :) For example, we could also look at the capabilities of the `corrplot` package.
