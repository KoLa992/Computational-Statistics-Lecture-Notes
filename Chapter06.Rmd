...
In case of a multiple regression model $y_i=\beta_0+\beta_1x_{1i}+...+\beta_kx_{ki}+\epsilon_i$, **if** our $\beta_j$ regression **coefficient estimates are not maximum likelihood estimates and the Gauss-Markov assumptions don't hold**, then the coefficient standard errors, p-values and confidence intervals are becoming invalid and not to be used to make any inference about the significance of explanatory variable effects on the target.<br>
In order **to save at least our confidence intervals** for the $\beta_j$ coefficients in an unfortunate case like this, we can **apply Bootstrap** simulations.

As a practical example, let's return to the COVID-19 mortality data one last time from <a href="Prelude05.html" target="_blank">Prelude 5</a>.
...
Next, we define the function that returns the coefficients of the bootstrap sub-samples for the multiple regression model defined above.
...
The results are roughly the same as the OLS (and MLE, see <a href="Chapter04.html" target="_blank">Section 8.1 of Chapter 4</a>) $97\%$ confidence intervals.
...
Now, the version of the Bootstrap method **covered in Sections 1-4** is called the **Nonparametric Bootstrap**, where random sub-samples of the same size as the original data are taken from the data with replacement.
...
Since variance is way larger than the mean, the **negative binomial distribution seems like a good assumption**. As a reminder see <a href="Chapter01.html" target="_blank">Section 5.4 of Chapter 1</a>.
...
A nice **alternative when Bootstrap is too expensive computationally is the Jackknife method**. It follows the **leave-one-out** approach to simulate the sampling distribution of a statistical parameter. This means that we want to **to calculate what we would get as our estimated statistical parameter if an observation were replaced by a new one and we do this with every observation we have**.

Let's see this principle in action to get a $97\%$ confidence interval for the *bias corrected standard deviation of daily internet usage times* based on the ESS dataset! Thankfully, we already have the sufficiently filtered data frame as `ess_small` from Section 1.

Now, as a **first step**, let's **calculate the corrected standard deviations when exactly one observation is dropped from the data**, and **loop this calculation with every observation that we have**. This way, we'll have sample size number of estimated **partial standard deviations**, i.e. standard deviations in case when one observation is dropped from the data.

```{r}
sample_size <- nrow(ess_small)
partial_sd <- rep(NA, sample_size) # an empty vector with sample size number of elements to store the partial sd values

for (i in 1:sample_size) {
  partial_sd[i] <- sd(ess_small$DailyNetUse[-i])
}

hist(partial_sd)
```

Great, we have the $n=1397$ partial standard deviations, and they look quite similar to each other  around the value of $133$ as the standard deviation does not really change if we leave out 1 observation, only if that 1 observation is a bit of an outlier, in that case it decreases. Hence the left-tailed distribution of these partials. However, these partial standard deviations are **just standard deviations when we leave out 1 observation, not when that 1 observation is replaced with a random other observation!** To correct for this behavior we need to **introduce so-called PSEUDOVALUES**.

The partial estimates for the standard deviation estimates are all based on samples of size $n-1$ and not $n$.<br>
Hence, they are slightly biased versions of the full-sample standard deviation, because removing a point changes the sample size and the estimator’s distribution. The **pseudovalues rescale and recenter the partials around the standard deviation calculated from the full sample to approximated what the standard deviation would be for a full sample of size $n$ if observation $i$ were replaced by a new one**.<br>
And the **jackknife estimate of the standard deviation is the mean of these pseudovalues**.

```{r}
full_sample_sd <- sd(ess_small$DailyNetUse)
pseudos <- (sample_size * full_sample_sd) - ((sample_size-1) * partial_sd)
jack_sd <- mean(pseudos)
jack_sd
```

Now, since the jackknife estimate of the bias corrected standard deviation is just a mean, we can get the **jackknife confidence interval** of $97\%$ with the general (i.e. confidence multiplier coming from t-distribution) **confidence interval formula for the mean** (introduced in <a href="Chapter04.html" target="_blank">Section 4 of Chapter 4</a>) applied on the mean of the pseudovalues. So, **in the standard error formula of $s/\sqrt{n}$, we need the standard deviation of the pseudovalues as $s$!**

```{r}
jack_se <- sd(pseudos)/sqrt(sample_size)
alpha <- 1-0.97
length_of_CI <- qt(1-alpha/2, df=sample_size-1) * jack_se

c(jack_sd - length_of_CI,
  jack_sd + length_of_CI)
```

So, the **standard deviation of daily internet usage times in the entire Hungarian population is likely between $122.7$ and $144.2$ minutes with $97\%$ probability**. This is almost exactly the **same as the nonparametric bootstrap confidence interval** we had in Section 3.<br>
So, **with the jackknife, we could achieve the results of a bootstrap confidence interval with way less computation** (only $n=1397$ standard deviations were calculated and NOT 10000) and **without any randomness**.

Now, let's organize our **jackknife** calculations **as a custom R function**! We need to replace the `ess_small$DailyNetUse` with a general input `data` parameter and the `sd` function with a general input statistical function called `statistic`, and the confidence level should be set as a third input called as `conf` for example.

The output should be a list with 4 elements:

- `est` = jackknife estimate of the examined statistical parameter, i.e. the mean of the pseudovalues
- `se` = jackknife standard error of the jackknife estimate
- `ci` = jackknife confidence interval calculated with the confidence interval formula for the mean
- `pseudos` = vector of the $n$ pseudovalues
- `partials` = vector of the $n$ leave-one-out partial estimates of the examined statistical parameter

Let's **write this custom function**, and use it to reproduce our $97\%$ jackknife confidence interval for the bias corrected standard deviation!

```{r}
jknife <- function(data, statistic, conf=0.95) {
  sample_size <- length(data) # data is assumed to be a vector, i.e. column of a data frame
  partials <- rep(NA, sample_size)
  
  for (i in 1:sample_size) {
    partials[i] <- statistic(data[-i])
  }
  
  full_sample_stat <- statistic(ess_small$DailyNetUse)
  pseudos <- (sample_size * full_sample_stat) - ((sample_size-1) * partials)
  jack_estimate <- mean(pseudos)
  
  jack_se <- sd(pseudos)/sqrt(sample_size)
  alpha <- 1-conf
  length_of_CI <- qt(1-alpha/2, df=sample_size-1) * jack_se
  
  CI <- c(jack_estimate - length_of_CI,
          jack_estimate + length_of_CI)
  
  return(list(est=jack_estimate,
              se=jack_se,
              ci=CI,
              pseudos=pseudos,
              partials=partials))
...
jack_result_sd <- jknife(ess_small$DailyNetUse, sd, 0.97) # testing the custom function
jack_result_sd$ci # see the confidence interval from the returned list
```

Great, we have the same $97\%$ confidence interval for the corrected standard deviation as before!

Now, let's use this new `jknife` of ours to show the **limitation of the jackknife compared to bootstrap**: it is **extremely poor for robust statistical parameters like the median** and other quantiles.<br>
This is perfectly understandable as **robust parameters are NOT sensitive to outliers, so leaving out one observation does not change their values**. So, we'll have the same partial estimates, which means the pseudovalues are also the same, so they do not have any variance to calculate confidence interval from. So, the jackknife confidence interval for the median will have the same lower and upper boundaries as the median calculated from all observations.

```{r}
jack_result_median <- jknife(ess_small$DailyNetUse, median, 0.97) # testing the custom function
jack_result_median$ci
```

Hmm, <a href="https://www.youtube.com/watch?v=NYvwLLxKqhM" target="_blank">f\*\*\*k</a>. The confidence interval bounds are truly the same. So, there is a limit to jackknifing, and in cases like these we need that extra computational power and randomness that the bootstrap provides to obtain reliable confidence intervals!