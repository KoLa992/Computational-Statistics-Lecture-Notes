...
As the sample size increases, the shape of the histogram gets closer to the density function of the underlying distribution.
...
With the help of these `r`-prefixed functions of probability distributions, I can simulate the behavior some famous theorems in R. Like the *Central Limit Theorem* (CLT).
...
- **Independent**: the **selection of sample elements is random**, meaning that what I select for the 3rd element **does not depend** on what I selected for the 1st and 2nd elements. In more technical terms this means that the covariance of any two $i \neq j$ observations is $0$: $cov(y_i,y_j)=0$
...
What we last look at in this chapter is how to **generate $10000$ IID samples of size $100$ from the population** of the Balaton swimmers.

The `replicate` function is going to be our main tool here.
