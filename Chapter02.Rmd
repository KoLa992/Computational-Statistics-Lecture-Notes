...
**Simulations** are the **common fundamental root for every advanced method in Computational Statistics**. So, in this chapter we review the basics of random number generation in R and explore how this can be used to

1. Simulate empirical data from any probability distribution
2. Generate different kinds of samples from observed population data
3. How 1+2 can be used to simulate the behavior of theorems like CLT

**These three points** covered in this chapter are going to be **utilized in Computational Statistics methods covered in subsequent chapters**.<br>
So, let's **start with** covering the **basics of random number generation** in R.

Every programming language includes a function that generates random numbers between $0$ and $1$. More precisely, these functions **generate numbers in such a way that they each have an equal probability of being any value between $0$ and $1$**, that is, they follow a **uniform distribution**.<br>
...
Node, **what does this random number generator do from a statistical point of view**? Well, in this case, they **randomly draw a $x$ value from a random variable $Y$, whose distribution is uniform between the $0$ and $1$ values**. The mathematical notation for this is: $Y \sim U(0,1)$. The $U$ comes from the fact that the *distribution is uniform*. :)

Now, let's draw $50$ random values from the $Y \sim U(0,1)$ random variable, and store the results in a `vector` object! We can also write that the $y_i$ observations ($i=1,2,...,50$) of this vector are all coming from a $U(0,1)$ distribution: $y_i \sim U(0,1)$<br>
That is, **we are taking a sample of size $n=50$ from the $U(0,1)$ distribution**.

Technically, we only need to change the `n` parameter of the function to take this sample.
...
**Let’s check this theory in practice!** As we know, in R, we calculate the quantile function for every distribution using a function that has the `q` prefix (see <a href="Chapter01.html" target="_blank">Chapter 1.</a>).
...
The $\lambda$ parameter for the Exponential distribution can be set in R’s `rate` parameter, for example, using the reciprocal of the mean. Since the expected value of the Exponential distribution is $\frac{1}{\lambda}$. For a reminder, see <a href="Chapter01.html" target="_blank">Section 4.1. of Chapter 1.</a>!
...
Great, based on the `str` function, we have all three $50$-element samples! With the `gather` function from the `tidyverse` package to convert it to a long format for `ggplot` visualization utilizing the `facet_wrap` layer just like we did in <a href="Chapter01.html" target="_blank">Sections 5 and 6. of Chapter 1</a>.

```{r}
library(tidyverse)
all_in_one_long <- all_in_one %>% gather("distribution", "sample_value")
all_in_one_long
```

So, let's view the result on a histogram with $k=6$ bins in `ggplot`. The `scales="free"` option in `facet_wrap` allows the subplots of the different distributions to have `x` and `y` axes and are not forced in the same `x` and `y` ranges.

```{r}
ggplot(all_in_one_long, aes(x=sample_value)) +
  geom_histogram(aes(y = after_stat(density)), bins=6) +
...
- $N(80,20)$ distribution
  ...
  * Median: the value of the quantile function at $0.5$
  * Probability of values smaller than 100: $P(y_i<100)=\int_{-\infty}^{100}{f(x)}dx$
...
Let’s **calculate these for each distribution** in separate `vector`s, then combine them into a data frame! The row indices of the data frame will be named after the calculated measure's name!
...
- The `mean` function then sums such an array with `TRUE = 1` and `FALSE = 0` and divides with total number of observations: in other words, it gives the *proportion of favorable observations*: the *the relative frequencies of values smaller than 100*.
...
### 2.4. The Glivenko–Cantelli Theorem

One of the conclusions drawn based on our previous simulations is actually so important that it is actually mathematical theorem, the **Glivenko–Cantelli Theorem** which says that as the **sample size increases, the shape of the histogram gets closer to the density function of the underlying distribution**.

Let's see what this theorem says a bit **more formally**. 

1. We have **a theoretical distribution** (Normal, Exponential, Binomial, Poisson, etc.) that is **given by its cumulative distribution function**, $F(x)$ (as defined in <a href="Chapter01.html" target="_blank">Section 2.2. of Chapter 1.</a>)
2. We have an **independent, identically distributed = IID sample** of size $n$ from this distribution with elements denoted as $y_1,y_2,...,y_i,...,y_n$. This IID assumption means two things for the sample observations $y_i$s.
    * **Sample elements are randomly drawn, so they are independent of each other**: what is drawn as the $i$th observation is not influenced by any other $i \neq j$ observations. More formally it can be expressed that the covariance (as seen in <a href="Prelude03.html" target="_blank">Section 7.2. of Prelude 3.</a>) of any two observations is $0$: $cov(y_i,y_j)=0, \forall i \neq j$
    * **Each** sample **observation is drawn from the same theoretical distribution**, so for every observation the $P(y_i<x)$ probability is defined by the same $F(x)$ distribution function given in the 1st point. That is $\forall i=\{1,2,...,n\}:P(y_i<x)=F(x)$
3. The in the observed sample, we can get the **cumulative relative frequencies** denoted by $F_n(x)$ which is the proportion of every $y_i$ observation smaller than an arbitrary $x$ number in the observed sample. In the $F_n(x)$ notation $n$ is the sample size in which these proportions were calculated.
4. The **statement of the Glivenko–Cantelli Theorem** with the notations and assumptions given in points 1-3. is the following: $\lim_{n\to\infty} |F_n(x)-F(x)|=0$ This simply means that the **empirical distribution $F_n(x)$ converges to the true theoretical distribution $F(x)$ as the sample size $n$ increases**.

Visually speaking, this theorem says that as the sample size increases ($n\to\infty$), the shape of the histogram (visual representation of $F_n(x)$) gets closer to the density function of the underlying distribution (visual representation of $F(x)$). However this statement is only true if the sample elements are randomly drawn (independent) from the same distribution (identical distribution), but this is exactly what the `r` prefixed functions of the theoretical distributions in R do. So, we are in the position of **simulating the inner workings of this theorem**.

Let's do this with a $LN(5.01,0.64)$ distribution (we could use any other distribution as well). So, we are drawing IID samples with the `rlnorm` function with sizes of $n=\{10,50,100,500\}$. And hopefully we'll see that the empirical histograms of these samples get closer and closer to the $LN(5.01,0.64)$ density function as the sample size ($n$) increases.

First, we are generating these 4 samples and store their values in a column of a data frame below each other, and in a separate column we denote which element is in which sample (n = 10, n = 50, n = 100, n = 500). The `rep` function will help a great deal in replicating these sample size labels in the data frame. This way, the simulated data frame is in a long format as seen in <a href="Chapter01.html" target="_blank">Sections 5 and 6. of Chapter 1</a>.

```{r}
mu <- 5.01
sigma <- 0.64

gc_df <- data.frame(n=c(rep("n = 10",10),
                        rep("n = 50",50),
                        rep("n = 100",100),
                        rep("n = 500",500)),
                    values = c(rlnorm(10, meanlog = mu, sdlog = sigma),
                               rlnorm(50, meanlog = mu, sdlog = sigma),
                               rlnorm(100, meanlog = mu, sdlog = sigma),
                               rlnorm(500, meanlog = mu, sdlog = sigma)))
gc_df
```

This long format is useful for visualizing the convergence of the histograms to the theoretical $LN(5.01,0.64)$ density function. We create the usual histogram+density plot with `ggplot` as seen in <a href="Chapter01.html" target="_blank">Chapter 1.</a> several times, then we add the `facet_wrap(~n)` layer to show the plot separately for every label given in the `n` column of our long data frame.

```{r warning=FALSE}
ggplot(gc_df, aes(x=values)) +
...
The inner workings of the **Glivenko–Cantelli Theorem** are shown rather nicely: **as sample size increases, the shape of the histogram gets closer to the density function of the underlying $LN(5.01,0.64)$ distribution**.

## 3. Simulating the Central Limit Theorem

With the help of these `r`-prefixed functions of probability distributions, I can simulate the behavior of some other famous theorems. Like the *Central Limit Theorem* (CLT).

The CLT states that

1. If I have a number of $n$ random variables denoted as $X_1,X_2,...,X_n$
2. that are independent from each other ($cov(X_i,X_j)=0,\forall i\neq j$)
3. have the same identical distribution ($X_1 \sim X_2 \sim ... \sim X_n$)
    * with $E(X_i)=\mu, \forall i$
    * and $Var(X_i)= \sigma^2, \forall i$
4. then $\sum_{i=1}^n{X_i} \sim N(n \mu, \sqrt{n}\sigma)$ if $n$ is "large enough", meaning as $n \rightarrow \infty$

In other words, the sum of IID random variables follows a normal distribution if the number of terms in the sum is large enough. Since we are talking about IID random variables just like in the Glivenko–Cantelli Theorem, the `r`-prefixed functions of probability distributions are perfect for simulating the terms of the $\sum_{i=1}^n{X_i}$ sum.

Ok, so let's see a specific version of this with $X_i \sim Exp(0.01), \forall i$ (we could use any other distribution as well) and $n=10, n=100, n=500$. We'll calculate these $\sum_{i=1}^n{X_i}$ sums with the different $n$s $1000$ times with the help of the `sapply` function.
...
n_50 <- sapply(1:1000, function(x) sum(rexp(n=50, rate = lam)))
...
Ok, see the mean and standard deviation for the $n=10$ case.

- As for the $X_i$ exponential distributions
  * $E(X_i)=\mu=1/\lambda=1/0.01=100$
  * $\sqrt{Var(X_i)}=\sigma=1/\lambda=1/0.01=100$
- So we expect the sums to have
  * $E(\sum_{i=1}^n{X_i})=n \mu=10 \times100=1000$
  * $\sqrt{Var(\sum_{i=1}^n{X_i})}=\sqrt{n}\sigma=\sqrt{10} \times 100=316.23$.
    - Due to the fact that $cov(X_i,X_j)=0$, the variances can simply be added up.
...
Ad we can see from the histograms that the simulated distributions for the $\sum_{i=1}^n{X_i}$ sums approximate the normal density function (bell curve) more accurately if the $n$ is larger. Before plotting the histograms, the data frame is converted to long format as seen in <a href="Chapter01.html" target="_blank">Sections 5 and 6. of Chapter 1</a>. I also fix the bin number on $15$.

```{r}
clt_df %>% gather("n", "sum_values") %>%
  ggplot(aes(x=sum_values)) +
...
So, what we did so far in this chapter, was the **sampling from theoretical distributions, known as Independent Identically Distributed (IID) sampling** in its maiden name.

Remember form Section 2.4. that the IID sample has two characteristics:

1. **Independent**: the **selection of sample elements is random**, meaning that what I select for the 3rd element **does not depend** on what I selected for the 1st and 2nd elements. In more technical terms this means that the covariance of any two $i \neq j$ observations is $0$: $cov(y_i,y_j)=0$
2. **Identically Distributed**: Each sample element comes from the **same distribution** (e.g., $N(80,20)$ or $Exp(0.0125)$), with no change in the distribution from which values are randomly drawn.
...
Looks like we have our $n=100$ IID sample! :)
...
- $\bar{y}=\hat{\mu}=160.81$ minutes
- $s=\hat{\sigma} = 36.74$ minutes
- $p=\hat{P} = 0.27=27\%$

These don't exactly match the true population values, but they are *not drastically different* from the reality. Again, we trust that the **difference between the estimated and true values is due to sampling error alone**.

### 4.1. The Method of Resampling

What we last look at in this chapter is how to **generate $10000$ IID samples of size $100$ from the population** of the Balaton swimmers. This technique is called as **resampling** (retaking a random sample several times) and it'll be the basis for lots of methods in Computational Statistics.

The `replicate` function is going to be our main tool here. It replicates (what a surprise...:)) a given R codes as many times as we want and stores the results in `matrix` format which is technically data frame that contains only numbers (basically the tabular format of a numeric vector). The function has two input parameters. The first is the number of times we want to replicated the code given in the second parameter of the function.

I want to replicate the `sample(swimming$TIME, size=100, replace = TRUE)` function `10000` times, so this is how I call the parameters of the `replicate` function.<br>
The  `sample(swimming$TIME, size=100, replace = TRUE)` function takes a sample of size 100 from the `TIME` column alone and stores the elements in a numeric vector of length 100. The `replicate` function returns this vector of 100 times in a `matrix` of 100 rows and 10000 columns. So, in this format sample observations are in the rows and samples are separated in the columns. For later usage, a more practical format for is the one where the sample observations are in the columns and samples are separated in the rows. So we transpose the result `matrix` of `replicate` with the `t()` function and convert it to `data frame` format.<br>
Finally, I just add the row and column names of the data frame as an optical tuning and remember that observations are in the columns and samples are separated in the rows.

```{r}
set.seed(1992)
samples <- replicate(10000, sample(swimming$TIME, size=100, replace = TRUE))
...
Very nice, so now I have 10000 IID samples of size 100 from the Balaton swimmers' time results! :) Notice that if I fix the random seed in the beginning of the code, I'll get the same 10000 samples each time, but the individual samples are different. This way the resampling code is reproducible without worrying that I get the same sample of size 100 10000 times. So the machine spirit is smart here. :)

Of course, I can do the same thing with any theoretical distribution here. So, I can easily get 10000 IID samples of size 200 from an $Exp(0.01)$ distribution the same way with the `replicate` function.

```{r}
set.seed(1992)
exp_samples <- replicate(10000, rexp(n = 200, rate = 0.01))
exp_samples <- as.data.frame(t(exp_samples))
rownames(exp_samples) <- paste0("Sample",1:10000)
colnames(exp_samples) <- paste0("Element",1:100)
head(exp_samples)
```

Let's check the distribution of let's say the 6666th sample. :) Here, I need to transpose the selected sample back as `ggplot` expects that observations (i.e. sample elements) are in the rows.

```{r}
ggplot(as.data.frame(t(exp_samples[6666,])), aes(x=Sample6666)) +
  geom_histogram(aes(y = after_stat(density))) +
...
All is perfect, praise the glory of our machine spirit again! :)