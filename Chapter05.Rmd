---
title: "Fundamentals of Hypothesis Testing and Multiple Comparisons"
...
### 5.4. GLRTs in Multiple Regression

Now, I think it comes as no surprise that these nice **GLRTs can also apply in multiple regression models** as well. So, let's circle back again to the multivariate regression model we estimated by classic OLS in <a href="Prelude04.html" target="_blank">Section 2 of Prelude 4</a>.

The data file used was <a href="https://github.com/KoLa992/Computational-Statistics-Lecture-Notes/blob/main/COVID_Deaths_Hungary.csv" target="_blank">COVID_Deaths_Hungary.csv</a> that stores data on 5 variables (columns) for 102 Hungarian districts (those where hospitals had average or below-average capacity utilization in 2019 according to [NEAK data](http://www.neak.gov.hu/data/cms1026624/Korhazi_agyszamkimutatas_2019.pdf)):
...
Great, we still know how to read *csv* files, so congrats to ourselves! :)

After we are done with celebrations, let's rerun the multiple regression model that predicts the COVID-19 mortality rate with **Nurses**, **Unemp** and **WomenOlder65** as explanatory (or predictor) variables.

```{r}
multivar_model <- lm(DeathsCOVID ~ Nurses + Unemp + WomenOlder65, data = COVID)
summary(multivar_model)
```

Now, let's also **reproduce the maximum likelihood estimators** of the coefficients and the residual standard deviation **under normality assumption on the residual term** as seen in <a href="Chapter04.html" target="_blank">Section 8 of Chapter 4</a>!

```{r}
# define the negative log-likelihood of the regression model
...
# do the maximum likelihood with optim
...
Great! Again, roughly the same as the OLS results as with confidence intervals in <a href="Chapter04.html" target="_blank">Section 8.1 of Chapter 4</a>.
...
An **alternative** for the $\chi^2$ test **if our numeric variable $Y$ has continuous distribution** is the **Kolmogorov-Smirnov test**. Here, you don't need to bin your continuous variable before applying the test, like we did with the $\chi^2$ test, as this test applies the continuous distributions' **cumulative distribution function** $F(x)$ directly. If You need a reminder on cumulative distribution functions, see <a href="Chapter01.html" target="_blank">Section 2.2 of Chapter 1</a>.

The $H_0$ and $H_1$ of the test are the same as for the $\chi^2$ test:

- $H_0$ states that our observed variable $Y$ follows the specified continuous distribution in the unobserved population
- $H_1$ stat that our observed variable $Y$ has a significantly different distribution than the one stated in $H_0$

So, in general if we want to see if our observed variable $Y$ follows a specific continuous distribution (normal, exponential, etc.), we need to take the fitted probability distribution's **theoretical cumulative distribution function $F(x)$** and we **compare** it with the **cumulative relative frequencies of the observed sample denoted as $F_n(x)$**. So, $F_n(x)$ is simply defined as follows: $$F_n(x)=\frac{\text{number of observations} \leq x}{n}$$

According to the Kolmogorov-Smirnov (**KS**) test statistic we compare $F(x)$ and $F_n(x)$ at their largest possible difference (in the *supreme* point of their absolute differences along the possible $x$ values the variable $Y$ can take): $$D_n=\sup_{x}{|F(x)-F_n(x)|}$$

This is illustrated on the following plot. The red line is $F(x)$, the blue line is $F_n(x)$, and the **black arrow is the KS statistic ic** itself.

<center>
![](KS_Example.png){width=40%}
</center>

The **p-value** is derived from the <a href="https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test#Kolmogorov_distribution" target="_blank">Kolmogorov distribution</a> in a right tailed way (since the best case of the test statistic for $H_0$ is $D_n=0$ of course, and we need to decrease the p-value if $D_n>0$). This is a special distribution developed just for this test alone, and due to the providence of the machine spirit, it is implemented in base R, so we don't really burden ourselves with its density function and other stuff. You can see it on the link before if You want. :)

So now that we know how the KS test works, let's see it in action and examine with this test as well whether the BMI values in the Hungarian ESS sample follow a normal distribution. So we have the following hypotheses.

- $H_0:$ The true population distribution is **normal**
- $H_1:$ The true population distribution is **NOT normal**

To obtain the test statistic and p-value, we need to apply the `ks.test` function in base R. The first parameter of the function is the observed numerical variable and the second is the R code for the theoretical cumulative distribution function ($F(x)$) of the distribution assumed in $H_0$. So, we need the `p` prefixed function of the normal distribution here. Thankfully the function is smart enough to estimate the parameters of the fitted distribution (so $\mu$ and $\sigma$ in this case) in the background via maximum likelihood method.

```{r warning=FALSE}
ess_small <- ess[!is.na(ess$BMI),c("idno", "BMI")]
ks.test(ess_small$BMI, "pnorm")
```

Our p-value is smaller than $2.2 \times 10^{-16}$, so this is a rejection of $H_0$ on every common significance levels. :( So, in this case the test says that the **BMI distribution is significantly different from the normal distribution even on $\alpha=1\%$.**<br>
This is a bit of a contradiction with the results of the $\chi^2$ test where on $\alpha=1\%$, the normality $H_0$ couldn't be rejected.<br>
The reason behind this contradiction is the fact that **the Kolmogorov-Smirnov test is even more in favor of $H_1$ than the $\chi^2$ test**. In the $\chi^2$ test, we examine the differences of observed and theoretical distribution in more then one points, in $k$ points to be exact. However, the KS test examines this difference in one point alone, where the theoretical and observed distribution shows the largest amount of difference. So, there is not even a minimal level of control on the other parts of the distribution that we have in the  $\chi^2$ test. This causes **more Type I errors in the KS test**, especially in large sample sizes. So, I **advise using $\chi^2$ test even for continuous distributions**. The extra work with forming the bins based on quantiles usually worth it as we can compare the obserevd and theoretical distributions in more than 1 point this way. Simulation studies show that a $\chi^2$ test with deciles or quintiles as bins have more power (i.e. less Type II errors) than the KS test as well.

Now, we have several legends surrounding the name of the  Kolmogorov-Smirnov test. The official version is that it is named after Soviet mathematicians Andrey Kolmogorov and Nikolai Smirnov, who developed it in the 1930s at their comfy stay in Hotel Gulag.<br>
However, the other version is that Andrey Kolmogorov got so drunk on *Smirnov vodka* that he came up with this test that has such an unfavorable Type I error rate compared to the $\chi^2$ test. What is the truth, we'll never know. :)

<center>
![](ks_meme.png){width=30%}
</center>
...
- $H_0:$ The two groups (males and females) have **identical BMI distributions** in the population
- $H_1:$ The two groups (males and females) have **different BMI distributions** in the population
...
The Kolmogorov–Smirnov (KS) test may also be used to test whether two underlying continuous probability distributions differ significantly. So, it has a version that mirrors the $\chi^2$ test of homogeneity for continuous numerical $Y$ variables specifically. This version is also called as the **two-sample KS test** since we are comparing the distribution of the variable $Y$ along two groups $A$ and $B$, and the two groups are treated as two independent samples in this case. Meaning that the groups mustn't overlap: if someone is in group $A$, they cannot be in group $B$ as well. The hypotheses of this test is the same as for the $\chi^2$ test of homogeneity.

- $H_0:$ The two groups ($A$ and $B$) have **identical distributions** for the variable $Y$ in the population
- $H_1:$ The two groups ($A$ and $B$) have **different distributions** for the variable $Y$ in the population

For the test statistic we just need the **cumulative relative frequencies of the two groups** $F_A(x)$ and $F_B(x)$. Both are defined the same way as $F_n(x)$ in Section 7.2. And by the KS logic, the test statistic is the supremum of differences between the two groups' cumulative relative frequencies along the possible $x$ values the variable $Y$ can take. $$D_n=\sup_{x}{|F_A(x)-F_B(x)|}$$

The p-value is derived from the same <a href="https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test#Kolmogorov_distribution" target="_blank">Kolmogorov distribution</a> in a right tailed way as it was for the one-sample case in Section 7.2.

Now that we know how the KS test of homogeneity works, let's apply it to test **if males and females have a significantly different BMI distributions** in the unobserved population. So, the two hypotheses are the same as in the $\chi^2$ test of homogeneity.

- $H_0:$ The two groups (males and females) have **identical BMI distributions** in the population
- $H_1:$ The two groups (males and females) have **different BMI distributions** in the population

The test statistic and p-value are obtained with the same `ks.test` function as before, but it is now used with the same `~` signed way as the `oneway.test` in Section 6.2. So, we need the following syntax: `numerical variable ~ variable that defines the two groups`. And in the `data` parameter we give the name of the data frame where these two variables live.

```{r warning=FALSE}
ess_small <- ess[!is.na(ess$BMI),c("idno", "BMI", "Sex")]
ks.test(BMI~Sex, data = ess_small)
```

In our case, the p-value is $1.101e-10$, which is smaller than even the lowest typical significance level of $1\%$.
So **in the population** (i.e., outside of our sample), **the BMI distribution of males and females can be considered DIFFERENT**.<br>
This comes as no surprise, since we rejected this same $H_0$ with the $\chi^2$ test too. And **even in this two-sample case, the KS test has the same drawback compared to the $\chi^2$ test as it had in the one-sample (or goodness-of-fit) case**. Namely, **the Kolmogorov-Smirnov test is even more in favor of $H_1$ than the $\chi^2$ test**. In the $\chi^2$ test, we examine the differences of the two groups' distribution in more then one points, in $k$ points to be exact. However, the KS test examines this difference in one point alone, where the two groups' distribution shows the largest amount of difference. So, there is not even a minimal level of control on the other parts of the distribution that we have in the  $\chi^2$ test. This causes **more Type I errors in the KS test**, especially in large sample sizes. So, I **advise using $\chi^2$ test even for continuous distributions in this case as well for the same reasons described in Section 7.2**.

## 8. Correction For Multiple Comparisons

The sad truth of statistical hypothesis testing that **even if we have** truly **random observed samples**, we can have **problems** in applying the "usual" statistical tests and methods we've covered in these previous sections **if the hypothesis is about deciding if a predictor variable has a significant effect on a target variable and this predictor is a nominal variable with multiple categories (groups)**.

We've discussed this issue already in Section 6.3, regarding the ANOVA test for comparing several group means that **as more statistical analyses are applied to the same observed sample, the Type I error rate** (rate of erroneously rejecting the null hypothesis) **increases**. In this context, **an ANOVA test for comparing $g$ group means can be considered as applying the two-sample t-test $g$ times on our numerical variable of interest**.
...
### 8.1. A Practical Example - Math Scores of 50 Schools
...
So, let's see exactly how many schools we have with p-values < 0.05

```{r}
pvals <- summary(lmod)$coef[,4]
coef(lmod)[pvals < 0.05]
```

Well, 20 out of 50! Yeah, seems a lot based on the box plot. However, from this result above, it seems that there are 20 schools with p-values that suggest we can reject the null hypothesis and feel 95% confident that the mean mathematics score for this school is significantly different from the overall mean. However, we have to account for how many comparisons are being made using the same sample of data. And this is where the **Bonferroni correction** comes into play, as Bonferroni Correction is the process of accounting for familywise error.

Because of the multiple comparisons (exactly $c=50$) made with the math score means among schools, adjustments must be made to either the $\alpha$ level that p-values are evaluated against, or to the p-values themselves.

### 8.2. Bonferroni correction

So, when should Bonferroni Correction be considered?

1. for multiple comparisons problems (comparing multiple means as in ANOVA)
2. multiple analysis problems (correlation matrices, multiple regression models run on the same target etc)

Now, let's see how this Bonferroni Correction is **calculated**. As mentioned before, we can either correct the significance level $\alpha$ or the p-values themselves.

**Method 1**: multiply the observed p-values by the number of comparisons being made and evaluate which p-values are below the desired  $\alpha$  after this correction.<br>
Let's see this way of calculation, and then check how many schools have Bonferroni corrected p-values that are statistically significant at $\alpha = 5\%$
...
After correcting for familywise error, we see that only 8 schools have mean mathematical scores that have a statistically significant difference in means from the global mean. And these are those 8 that we'd have expected based on the box plot of Section 8.1 as well.
...
Great, these are the same schools as before!

And of course, we have a built-in function for this called `p.adjust`. As the name suggests, it corrects the p-values.

```{r}
padj <- p.adjust(pvals, method = 'bonferroni')
coef(lmod)[padj < alpha]
```

Great, same 8 schools as with the solutions earlier! So, the rest of the schools that initially showed a significantly different mean math score from the rest at $\alpha=5\%$ were just familywise errors caused by the $c=50$ comparisons made during the partial t-test of the dummy variables in the regression model.

So, this Bonferroni correction can nicely "kill" those p-values that are in the range of common significance levels of $1\%-10\%$ (where it's best to make no decision about $H_0$ according to Section 1) if they are just a product of familywise error and $H_0$ actually couldn't be rejected in those cases at the selected $\alpha=5\%$ levels.<br>
Finally, here's a meme to remember all that. :)

<center>
![](bonferroni_meme.png){width=35%}
</center>