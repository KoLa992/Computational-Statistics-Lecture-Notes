...
#### Applications of the Fitted Exponential Distribution 
...
- $\mu = \log(\hat{Me})$
- $\sigma = \sqrt{2\left(\log(\bar{y})-log(\hat{Me})\right)}$
...
sigma <- sqrt(2*(log(sample_mean)-log(sample_median)))
c(mu,sigma)
...
<a href="https://youtu.be/y13n1CeyqjY?t=8" target="_blank">Huzzah</a>! Looking at the two plots we can see that this new $LN(5.01,0.64)$ distribution is a way better fit for the observed daily net usage time data then the previous lognormal was! According to the histogram, the mode (place of the maximum for the density curve) is in the right place, and the maximum value is in the sample place as the maximum value of the empirical histogram. The empirical and theoretical quantiles also match rather nicely: even in the higher ranges above 500-600 minutes, the theoretical quantiles underestimation of the empirical ones is not very substantial. And again, we need to note that these higher ranges are on the right tail of the distribution where we have very few observed data, so in these ranges the theoretical distribution is probably more reliable due to the fact that we don't have many empirical observations to work with in this part of the distribution.<br>
So, overall, let's accept this $LN(5.01,0.64)$ distribution as the population distribution for the unobserved Hungarian's daily net usage time and do some calculations with it!

#### Applications of the Fitted Lognormal Distribution
...
Okay, looks like in Hungary a bit more than $1/3$ of the population, $36.4\%$ is like this. Probably the substantial elderly population is more hostile toward the online world than in other European countries. Maybe a lognormal distribution fit by different age groups might reveal if this hypothesis is true. :)
...
That is $341.7$ minutes, so roughly 5.7 hours! That's not so bad for the top 10% considering that this variable probably includes the time someone uses the net for work. This is probably a correct and not a conservative estimation since according to the Q-Q Plot the fit of theoretical quantiles to empirical ones is almost perfect for quantiles lower than 500 minutes.
...
- Long right tailed distributions
  ...
  * <a href="https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Chisquare.html" target="_blank">Chi-Squared</a>
  * <a href="https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Fdist.html" target="_blank">F</a>
...
The `num_claims` data frame again contains observations of individual cars, $n=500$ of them, but the `NumClaims` variable contains the number of accidents (or claims) they reported in a year. This means that this variable can take integers from $0$ to $+\infty$, as in theory there is no hard maximum on the number of accidents a car can have in a year. So there is no finite value on the Binomial distribution's $m$ parameter. This scenario is the case of the **Poisson distribution** that is the sum of $\infty$ $I(p)$ distributions or in other words it is the limiting distribution of the $B(m,p)$ if $m \rightarrow \infty$.

```{r}
str(num_claims)
```

The Poisson distribution has only one parameter denoted as $\lambda$ that determines the exact shape of its PMF. Hence, the Poisson distributoon is denoted as $P(\lambda)$ and if $Y \sim P(\lambda)$, then its PMF looks like the following. $$P(y_i=x)=\frac{\lambda^xe^{-\lambda}}{x!}$$

#### Application of Method of Moments (MM)

The first two moments of the Poisson distribution are rather simple since bots its expected value and variance are equal to $\lambda$. So, if $Y \sim P(\lambda)$, then we have that $$M_1=M_2=E(Y)=D^2(Y)=\lambda$$

So, the MM principle uses only $M_1$ for fitting the distribution since it has only 1 parameter, just like in the Exponential case in Section 4.1. So we have from $\hat{M_1}=M_1$ that the empirical mean of the sample is our $\lambda$ parameter as well. $$\bar{y}=\lambda$$

```{r}
lambda <- mean(num_claims$NumClaims)
lambda
```

So, the number of claims a year follows a $P(0.71)$ distribution.

It is important to note that due to its theoretical properties, the Poisson distribution is only a realistic fit for a discrete variable if its empirical mean and variance are close to each other. Let's check this for our data before we go on.

```{r}
c(mean(num_claims$NumClaims), sd(num_claims$NumClaims)^2)
```

Since $0.71 \approx 0.68$, we can accept that in the case of unobserved data, the theoretical property of the Poisson distribution ($E(Y)=D^2(Y)$) holds.

#### Checking the Goodness of Fit

Checking the goodness of fit for our $P(0.71)$ distribution can be done the same way as for the Binomial distribution. We compare theoretical $P(y_i=x)$ probabilities and relative frequencies for every observed value of the variable in our sample.

First we can get the relative frequencies (empirical probabilities) the same way we did in Section 5.2.
...
Of course, the Poisson distribution has the same `d`, `p` and `q` prefixed functions the Binomial has, and the work the same way, we just need to input 1 distribution parameter, $\lambda$ instead of the Binomial's two parameters.<br>
So, the theoretical $P(y_i=x)$ probabilities are easily obtained with the `dpois` function.

```{r}
freq_table$theoretical_probs <- dpois(freq_table$variable_values, lambda = lambda)
freq_table
```

Looks like the empirical and theoretical probabilities are quite close to each other. I'd say, there is no under or over represented value here compared to the theoretical probabilities.<br>
We can simple visualize these empirical and theoretical probabilities on a simple bar plot completely the same way as in Section 5.2.
...
Our nice little visualization confirms that the empirical and theoretical $P(0.71)$ probabilities are rather close to each other.

#### Applications of the Fitted Poisson Distribution

Now that the goodness of fit of the Poisson distribution is confirmed, we can calculate what is the probability of more claims occurring on a random car than the observed maximum of $4$ in the sample. Of course, we need the `ppois` function for this!

```{r}
1 - ppois(4, lambda = lambda)
```

The result is $0.084\%$. This is a bit higher than what we had in the Binomial case ($0.05\%$), but this is no surprise as we have no theoretical maximum value in the Poisson distribution. Even $\infty$ can happen with a positive probability, just a very-very low one.

Out of curiosity, we can obtain the value that is the minimum number of accidents (claims) for the riskiest 5% of cars. So, we are looking for the number where we have a 95% probability that we find a car with a lower number of claims. As expected, the `qpois` function works here.

```{r}
qpois(0.95, lambda = lambda)
```

The answer is $2$, which is again not surprise looking at the sums of the theoretical $P(y_i=x)$ probabilities in the `freq_table`. So, looks like the theoretical maximum value of the Binomial distribution does not cause the 95th percentile to change compared to the Poisson distribution. This is again no surprise since we discussed in Sections 5.2. and 5.1. quantiles are not informative for discrete distributions due to their low value sets.

### 5.4. Negative Binomial Distribution

So far we can say for discrete variables that are not binary (i.e. not Bernoulli) that if the empirical variance ($s^2$) is lower than the mean ($\bar{y}$) in the observed data, then we should try to fit a $B(m,p)$ distribution, and if these two measures are roughly the same, then try to fit a $P(\lambda)$ distribution.<br>
But what about the case when in our observed sample we have a $s^2>\bar{y}$ situation? This is when the **Negative Binomial distribution** comes into play.

The Negative Binomial distribution arises as a **mixture of Poisson distributions with different $\lambda$ parameters**. So if we are examining a discrete variable with let's say 3 groups that have Poisson distributions with different $\lambda_1, \lambda_2, \lambda_3$ parameters, then when examining this variable for all 3 groups included together, the overall distribution of the variable is Negative Binomial.

A practical example for this is on the *CarInsurance_NumberOfClaims_v2* sheet of the <a href="https://github.com/KoLa992/Computational-Statistics-Lecture-Notes/blob/main/insurance_examples.xlsx" target="_blank">insurance_examples.xlsx</a> file. Here we again have $n=400$ observation of insured cars and the `NumClaims` variable contains the number of accidents (or claims) reported in a year. However, in this case let's say we have young drivers who are quite risky, so their claim number is Poisson with a quite high $\lambda_1$ average claim number (Remember that if $Y \sim P(\lambda_1)$, then $E(Y)=\lambda_1$). Then we have middle-aged drivers with experience, so their claim number also follows a Poisson distribution, but with the lowest $\lambda_2$ average claim number. For elderly drivers the claim number still has a Poisson distribution but with a $\lambda_3$ average claim number that is in between $\lambda_1$ and $\lambda_2$ as elderly drivers are riskier for their slower reflexes but it is compensated with their experience. So, if we have these 3 driver groups and we are examining all 3 combined in a variable containing the number of claims without knowing who belongs to which group, then this combined variable follows a Negative Binomial distribution. To confirm this, let's read the *CarInsurance_NumberOfClaims_v2* sheet to an R data frame and examine the empirical mean and variance of the `NumClaims` variable.

```{r}
num_claims <- read_excel("insurance_examples.xlsx", sheet = "CarInsurance_NumberOfClaims_v2")
str(num_claims)

sd(num_claims$NumClaims)^2 > mean(num_claims$NumClaims)
```

Great, as expected, we have that $s^2>\bar{y}$, so we can go ahead with fitting a Negative Binomial distribution. :)

The Negative Binomial distribution has two parameters defining the exact behavior of its PMF: $r,p$. In R, they are called similarly as the parameters of the classic Binomial distribution. So, $r$ is called `size` and $p$ is called `prob`. However, for the Negative Binomial $r$ can be any positive real number, it is NOT restricted to positive integers! So, if if $Y \sim NB(r,p)$, then the PMF is as follows. $$P(y_i=x)=\frac{\Gamma(x+r)}{\Gamma(r)x!}p^n(1-p)^x$$

In the formula $\Gamma()$ is the <a href="https://en.wikipedia.org/wiki/Gamma_function" target="_blank">gamma function</a>

#### Application of Method of Moments (MM)

To get the $r,p$ parameters from the observed data with the MM principle, we need the first two moments as we have two parameters to determine. For a $Y \sim NB(r,p)$ variable, the first two theoretical moments, expressed with the help of the $r,p$ parameters, are the following.

- $M_1 = E(Y) = \frac{r(1-p)}{p}$
- $M_2 = D^2(Y) = \frac{r(1-p)}{p^2}$

So, applying the MM principle of $\hat{M_1}=\bar{y}=M_1$ and $\hat{M_2}=s^2=M_2$ gives us the following system of equations to solve for $r,p$.

- $\bar{y} = \frac{r(1-p)}{p}$
- $s^2 = \frac{r(1-p)}{p^2}$

From the 1st equation we can express that $\bar{y}p=r(1-p)$ and from the 2nd one we have that $s^2p^2=r(1-p)$. This also means that $\bar{y}p=s^2p^2$. And this way we have that $$p=\frac{\bar{y}}{s^2}$$

And once we have a result for $p$ we can have $r$ from the 1st equation: $$r=\frac{\bar{y}p}{1-p}$$

In R it is quite easy to substitute to these obtained formulas for $r,p$.

```{r}
sample_mean <- mean(num_claims$NumClaims)
sample_variance <- sd(num_claims$NumClaims)^2
...
So we have that the number of claims in this case follow a $NB(1.73, 0.52)$ distribution.

#### Checking the Goodness of Fit

Since the Negative Binomial distribution has the usual prefixed functions in R: `dnbinom`, `pnbinom` and `qnbinom`, we can check the goodness of fit by comparing the empirical relative frequencies with the theoretical $P(y_i=x)$ probabilities of the $NB(1.73, 0.52)$ distribution.

The technical solutions for this are completely the same as in Sections 5.2. and 5.3, so all the relevant code is in one unified chunk here. The only difference compared to the previous sections is in line 5, where the `dnbinom` is applied for the theoretical probabilities.
...
freq_table <- data.frame("variable_values"=variable_values,
                         "empirical_probs" = as.numeric(empirical_probs))
freq_table$theoretical_probs <- dnbinom(freq_table$variable_values, size = r, prob = p)
...
Our nice little visualization confirms that the empirical and theoretical $NB(1.73, 0.52)$ probabilities are rather close to each other.

#### Applications of the Fitted Negative Binomial Distribution

Now that the goodness of fit of the Poisson distribution is confirmed, we can calculate what is the probability of more claims occurring on a random car than the observed maximum of $9$ in the sample. Of course, we need the `pnbinom` function for this!

```{r}
1 - pnbinom(9, size = r, prob = p)
```

The result is $P(y_i>9) = 0.27\%$. This is quite a high value. Like for our $P(0.71)$ distribution $P(y_i>4)$ was just $0.084\%$. This is coming from the Negative Binomial distribution has the highest variance out of the discrete distributions examined so far. This high variance makes extremely high values to occur with a greater probability.

This high variance also means that the value that is the minimum number of accidents (claims) for the riskiest 5% of cars also increases. So, we are looking for the number where we have a 95% probability that we find a car with a lower number of claims. As expected, the `qnbinom` function works here.

```{r}
qnbinom(0.95, size = r, prob = p)
```

The result of $5$ is way higher than the $2$s we've seen before as expected from the higher variance of the Negative Binomial distribution. So the insurance company needs to prepare for a minimum of 5 claims for the riskiest 5% in this model compared to the 2 claims of the simpler Poisson model. So the multiple groups of drivers hidden in this variable complicate the homogeneous Poisson case. :)

## 6. Case Study - Ambulance Arrival Times

Okay, so after getting familiar with theoretical probability distributions and their usefulness compared to analyzing observed sample data alone, let's examine a **real-world case study for the Hungarian National Ambulance Service** where all of the knowledge acquired in this chapter (fitting distributions with MM, checking goodness of fit and then apply the fitted distribution's distribution and quantile functions to answer questions about unobserved population data) can be applied to **estimate ambulance arrival times**.

On 10th February 2025, an online Hungarian blog focused on healthcare <a href="https://rontgen.444.hu/2025/02/10/bozotharc-aran-sikerult-megszerezni-az-orszagos-mentoszolgalat-kierkezesi-statisztikajat-a-szamoktol-nem-lesz-boldog-senki" target="_blank">reported</a> the statistics of ambulance arrival times from the National Ambulance Service (NAS). Characteristic of Eastern European realities, data that in England would be freely downloadable, continuously updated, and accompanied by explanations on the ambulance service’s <a href="https://www.england.nhs.uk/statistics/statistical-work-areas/ambulance-quality-indicators/" target="_blank">own website</a>, in Hungary must be obtained by a blog through a formal data request.<br>
However, there is a twist in the story: the NAS provided only the mean and median (that is, the time within which half of the cases are reached). The typical 90th percentile used in quality assessment of ambulance services (the time within which 90% of cases are reached) was not disclosed. This, however, presents an interesting statistical problem: if we know the theoretical distribution of the arrival times, and it is a distribution defined by its mean and median, then from these two values we can compute the undisclosed 90th percentile! In fact, such problems are exactly what we have been solving throughout this chapter. Of course, we cannot know the true theoretical distribution in Hungary, but based on literature and statistical reasoning, we can make reasonable assumptions. In this case study, we are assuming that these **arrival times follow a lognormal distribution**. On the one hand, <a href="https://doi.org/10.3390/mca26040070" target="_blank">Pury (2021)</a> concluded that the long-right-tailed nature of the lognormal distribution is a good fit for modeling ambulance arrival times. On the other hand, as shown in Section 4.2, the parameters $\mu,\sigma$ of a lognormal distribution can be estimated from the sample mean and median using the principle of the method of moments.

The task is thus set. But first, let us examine the available data contained in the <a href="https://github.com/KoLa992/Computational-Statistics-Lecture-Notes/blob/main/ambulance_arrival.xlsx" target="_blank">ambulance_arrival.xlsx</a> file.

```{r}
response_times <- read_excel("ambulance_arrival.xlsx")
str(response_times)
```

So, from the structure of the data frame we can see that we have the following 5 variables:

- DATE: The date on which the data was collected. This is data on year-month level, where the day always marks the first day of the month.
- CATEGORY: The priority category of the calls. It has values from P1 (highest priority: serious injury) to P5 (lowest priority: no emergency, only patient transport). It also has an extra category called High P1, which is for life-or-death situations. Naturally, arrival times are influenced by this priority category. An ambulance needs to arrive to a High P1 case than to a P5 case. So we must break the 90th percentiles down to these categories as well.
- GEO: Indicating that the reported data is for Budapest only or nationwide.
- METRIC: Whether the data value reported is an average/mean ($\bar{y}$) or a median ($\hat{Me}$).
- VALUE: The exact numerical value reported for the Date+Category+Geo+Metric combination of the given row.

This means that our data frame is in a so called **long format**. That means that all the numerical data is collected in one column (VALUE) and we have an additional column that describes what that numerical value actually is (METRIC). First, we need to **transform** this data frame **into a wide format** where we have two separate columns for the average and the median. This can be done with the help of the `spread` function in the `tidyverse` package.

```{r}
response_times <- response_times %>% spread(METRIC, VALUE)
response_times
```

Once we have the data in a form where we have the observed empirical averages ($\bar{y}$) and medians ($\hat{Me}$) in separate columns, we can easily get the lognormal $\mu,\sigma$ parameters using the formula derived in Section 4.2.

- $\mu = \log(\hat{Me})$
- $\sigma = \sqrt{2\left(\log(\bar{y})-log(\hat{Me})\right)}$

We just need to substitute in these in R and save the results as two new columns for the two parameters.

```{r}
response_times$mu <- log(response_times$Median)
response_times$sigma <- sqrt(2*(log(response_times$Average) - log(response_times$Median)))
response_times
```

Unfortunately, we cannot check the goodness of fit for the fitted lognormal distributions as the individual-level empirical data is not published, only the mean and median are available that we've used for fitting the lognormal distribution.

Once we have the parameters of the lognormal distribution for every DATE-CATEGORY-GEO combination in our data frame, we can calculate the 90th percentile we are looking for in every case with the `qlnorm` function.

```{r}
response_times$P90 <- qlnorm(0.9, meanlog = response_times$mu, sdlog = response_times$sigma)
response_times
```

Ok, seems we have the results! In the 1st row, for example, we can see that 90% of ambulances arrived within $39.7$ minutes for High P1 (most urgent) cases in Budapest in 2018 November. Nationwide the result is a bit better (2nd row): only $32.4$ minutes is the time within which 90% of cases is served.<br>
Is this result good or bad? Well, <a href="https://www.england.nhs.uk/statistics/wp-content/uploads/sites/2/2025/01/20250117-AmbSYS-specification.pdf" target="_blank">according to the English Ambulance Service</a>, the **90th percentile should be under 15 minutes, meaning that at least 90% of cases should be reached within 15 minutes**. So, if we compare our results to English standards, the 90th percentile of Hungarian arrival time is more than double of the required time. Unpleasant.

But we can say that okay, this is a very old, 2018 data. Surely, the results have improved since then! So, let's visualize the results as a time series on line chart with different colors for the different emergency categories. And let's also add the rarget value of 15 minutes as a dotted horizontal line. Finally, let's create a separate plot for Budapest and Nationwide data by adding `facet_wrap(~GEO)` in the lat layer of our `gpplot` code.

```{r}
ggplot(response_times, aes(x=DATE, y=P90, color=CATEGORY)) +
  geom_line(linewidth=1) +
  geom_hline(yintercept = 15, linetype='dotted') +
  facet_wrap(~GEO)
```

On the one hand, we can say based on the plots that the 90th percentile of arrival times improved over time in the two most urgent categories (High P1 and P1). They decreased from roughly 40-45 minutes to roughly 18-20 minutes. In February 2023 and May 2021 they were even below 15 minutes in the High P1 category in Budapest! But this is just two months and just in Budapest.<br>
However, in the P2 and P3 categories the 90th percentile even increased quite significantly after 2020. In Budapest, the P3 category was around 150 minutes until 2024, but in 2024 there is a substantial decrease to around 100 minutes. We only have data for the P4 and P5 categories in 2024. Truth be told, in the English standards only 120 minutes are expected for the 90th percentile in the P3 category, so the results after 2024 are ok in that regard. Nationwide data is even lower thankfully!

So overall, there is room to improve to reach the English standard of 15 minutes in the High P1 and P1 categories.