...
Our p-values always decrease with increasing sample size. Due to technical null hypothesis stating equality and to the fact that our estimators are consistent.
...
That means that Type I error rates are increasing as sample size increases and Type II error rates are decreasing with a fixed $\alpha$ level.

```{r}
sample_size_list <- c(50,200,500,1000,2000,5000,10000)
...
Load ESS 2024 data to R data frame.
...
Let's assume that $BMI \sim N(\mu,\sigma)$, omit the missing values and check our normality assumption on a histogram.
...
Our hypothesis is that $\mu=25$. So we have that

- $H_0: \mu = 25$
- $H_1: \mu \neq 25$

According to the likelihood ratio test statistic we have that $$-2[l(25,\sigma)-l(\mu,\sigma)]$$

So, let's calculate $l(25,\sigma)$ with `optim`.
...
neg_log_likelihood_norm_H0 <- function(sd_norm){
  log_lik_observations <- log(dnorm(x = ess_small$BMI,
                                    mean = 25,
                                    sd = sd_norm)) # log-probability of every observations occurring under the current parameters
...
And now go for $l(\mu,\sigma)$ the same way, the difference is that $\mu$ is not restricted to $25$.
...
log_likelihood_H1 <- -norm_ml_H1$val
log_likelihood_H1
```

And the likelihood ratio (LR) test statistic is simply calculated as a difference times -2 at this point.
...
In case of a true $H_0$, this LR test statistic follows a $\chi^2(1)$ distribution (for many samples) as the number of restrictions in $H_0$ is only $1$ as we only gave a restriction for the $\mu$ parameter of the normal distribution. So, we can get the p-value from the $\chi^2(1)$ distribution in a right-tailed way.
...
This is a hard $H_0$ reject on every common $\alpha$s.

Let's see the same with a t-test.

```{r}
t.test(ess_small$BMI, mu = 25, alternative = "two.sided")
```

Let's see the $\mu=26.1$ statement.

First with the LR test.
...
And the same for the t-test.

```{r}
t.test(ess_small$BMI, mu = 26.1, alternative = "two.sided")
```

Wow, same p-value! As $t(\infty)=N(0,1)$ and by the definition $\chi^2(k)=\sum_{i=1}^k{N(0,1)}$, so for $k=1$ we have that $$t(\infty)=N(0,1)=\chi^2(1)$$
...
Next to these special cases, we can construct LR test to decide if an $Exp(\lambda)$ or a $LN(\mu,\sigma)$ is a better fit on the daily net usage time variable of the ESS 2024 data. Our hypotheses here are the following.
...
Filter to the valid Net Usage Time values.
...
To construct the LR test statistic by doing MLE for the two distributions, then subtract the maximized $l(\mu,\sigma)$ from the maximized $l(\lambda)$, and multiply the result by -2.
...
And p-value is again coming from a $\chi^2(1)$ distribution in a right-tailed way as the number of restriction is again $1$, as the simpler (restricted) exponential distribution has $1$ less parameter than the lognormal distribution.

```{r}
1 - pchisq(test_stat_LR, df = 1)
```

That's a hard $H_0$ reject on every common significance levels. So, the unrestricted lognormal distribution is a better fit on the data whe considering the whole population according to our LR test.