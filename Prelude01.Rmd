...
The latest version of R is <a href="https://cran.r-project.org/" target="_blank">R 4.5.1</a> that can be downloaded and is free to use. R also has a free to use integrated development environment (IDE) called <a href="https://rstudio.com/products/rstudio/download/) target="_blank">R Studio</a> that makes writing R program code easier. For example, it has automatic code completion. We'll use this environment in this course.
...
As I'm sure that duting your bachelor studies some kind of *Programming* courses mentioned that today's computers are operating on the **Neumann model**.
...
In our course, it won't have any significance, but if you need to handle true **big data**, then in order to save RAM storage, even the difference between integers and doubles matter! So, you don't want to use up the space needed for doubles to store something that can only be an integer number!

Some other important data types and their definitions:
...
Note that the `order` function has a second parameter that gives the direction of the ordering. The parameters, or in other words *arguments* of a function can be found in the <a href="https://www.rdocumentation.org/" target="_blank">online documentation for the R language</a>. You should just type in the function that you're looking for in the search bar.
...
If we download *csv* datasets from English websites like <a href="https://www.kaggle.com/" target="_blank">Kaggle</a>, then they probably satisfy all three conditions given above, but it's better to be safe.
...
The <a href="https://github.com/KoLa992/Computational-Statistics-Lecture-Notes/blob/main/COVID_0326.csv" target="_blank">COVID_0326.csv</a> file satisfies every technical and logical requirements. It contains $100$ countries as observations and we have $9$ variables descriving these countries:
...
```{r eval=FALSE}
COVID <- read.csv("https://raw.githubusercontent.com/KoLa992/Computational-Statistics-Lecture-Notes/main/COVID_0326.csv")
```
...
As I wrote in the introduction, one of the most attractive features of R is that its functionality can be extended by using external libraries called *packages*. These packages add extra functions to the command set of R that are not included in the 'base' version.
...
Let’s see a simple example! Let's write our own silly function called `avg`, which *returns the average of two numbers*. <br>
...
Finally, we revise the most fundamental concept of descriptive statistics in our new favorite tool, R: **the scales of measurement for statistical variables**. :)

Scales of measurement is defined on variables of a data frame. Essentially, they are a similar concept of data types, but with a more mathematical and statistical meaning.

Basically, variables can be sorted to the following $4$ scales of measurement:
...
The difference between the $4$ scales is what kind of mathematical operations ($=, \neq, >, <, +,-,\times,/$) are interpretable on the elements of the variable.

#### Nominal Scale

Variables on nominal scale in statistics are the ones which have a data type of `character` in R. In these variables, the **values are essentially just verbal description with no mathematical meaning at all**. The only mathematical operations that are interpretable here are just $=, \neq$ an that's it.

Some examples are: town names, gender, etc.<br>
Code numbers with no mathematical meaning are also on nominal scale: zip codes, or genders encoded as *1=male* or *0=female*<br>
Since for example for genders $1>0$ doesn't mean that males are higher than females or $1+0=1$ doesn't mean that a male plus a female equals a male.<br>
Although, encodings like this are useful sometimes, as the sum of a variable where *1=male* or *0=female* equals the number of $1$s = males in the dataset (or population).

so, there are is not even hierarchy between the values, not even if they are encoded numerically.

#### Ordinal Scale

For these variables the operations of $=, \neq, >, <$ are interpretable, but nothing more. So for these variables, we **know the order between the values, but not the distance**.

Most common examples are rankings on a race. As for example, if we see the ranking of a Formula 1 race and we see that Verstappen is 1st and Hamilton is 2nd , we know that Verstappen is better than Hamilton, but we do not know from this by how much!

Another example for ordinal scales are `character` variables in R, where the descriptions can be naturally ordered. Like levels of a degree decribed as "Acceptable, Good, Excellent". We know that Excellent is better than Acceptable, but we do not know by how much.

Furthermore, some `numerical` variables can fall here as well. Like if we rate a film with integer numbers from $1$ to $5$. We know that $5>3$, but we cannot be sure that in every viewers' mind a $5$ is higher than $3$ by $2$. So the distance between the values depends on the subjective perception of each individual, so $+,-,\times,/$ oprations are not safe to use here!

#### Interval Scale

Only variables with `numerical` data types can fall into this category. Here, **distance between numbers is interpretable, so operations like $=, \neq, >, <, +, -$ can be used on this scale**. However **the $0$ for the values in these variables is arbitrary not absolute**, so $\times, /$ operations cannot be applied.

Like when measuring temperatures: on Celsius scale $0$ is the temperature where still water freezes; on Fahrenheit scale $0$ is the temperature where the river Mississippi freezes.<br>
If we measure $10$ degrees in the morning and $20$ degrees in the afternoon on Celsius scale, we can say that the afternoon is $20-10=10$ degrees warmer than the morning. And we can also say that the afternoon is $\frac{20}{10}=2$ times warmer than the morning.<br>
Now, let's say that you have a temperature degree where $0$ is $+50$ degrees Celsius. Then the morning and afternoon temperatures become $-40$ and $-30$ respectively. The difference stays the same since $-30-(-40)=10$, but the ratio of the two temperatures will differ $\frac{-30}{-40}=0.75$.<br>
Hence, $\times, /$ operations are not interpretable for a variable on this scale.

Other common variables on interval scale are for example, dates ($0$ of the Gregorian calendar is the birth of Christ, $0$ of Muslim calendar is he migration of Muhammad) and height above sea level.

#### Ratio Scale

Only variables with `numerical` data types can fall into this category. Here, **distance between numbers is interpretable and these variables have an absolute $0$ point as well**. This way, **all mathematical operations $=, \neq, >, <, +, -, \times, /$ are interpretable on this scale**.

Some examples are monthly income of employees, population of countries, revenue of companies, etc.

#### The Case of the COVID data frame

If we take a look at the variables of the COVID data frame, we can quickly decide that the `ConutryName`, `ConutryName`, and `ConutryName` variables are on **nominal** scale and the all the numerical variables are fortunately on **ratio** scale:
